...
static void pppoe_recv_PADI(struct pppoe_serv_t *serv, uint8_t *pack, int size)
{
	...
	if (serv->accept_null_sn && (service_name_tag == NULL || service_name_tag->tag_len == 0)) {
		service_match = 1;
	}
...
//added option for accept-null-sn=0
static int parse_server(const char *opt, int *accept_null_sn, int *padi_limit, struct ap_net **net)
{
	char *ptr, *endptr;
	char name[64];
		...
		if (!strncmp(opt, "padi-limit=", sizeof("padi-limit=") - 1)) {
			*padi_limit = strtol(ptr + 1, &endptr, 10);
			if (*endptr != 0 && *endptr != ',')
				goto out_err;
			opt = endptr;
		} else if (!strncmp(opt, "accept-null-sn=", sizeof("accept-null-sn=") - 1)) {
			*accept_null_sn = strtol(ptr + 1, &endptr, 10);
			if (*endptr != 0 && *endptr != ',')
				goto out_err;
			opt = endptr; 
...
static void __pppoe_server_start(const char *ifname, const char *opt, void *cli, int parent_ifindex, int vid, int vlan_mon)
{
	...
	int accept_null_sn = 0;
	struct ap_net *net = def_net;

	if (parse_server(opt, &accept_null_sn, &padi_limit, &net)) {
		if (cli)
	...
	serv->vid = vid;
	serv->net = net;
	serv->accept_null_sn = accept_null_sn?1:0;
